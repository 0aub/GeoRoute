...
\begin{description}
    \item[AI-Powered Route Generation] allows users to simply mark friendly and enemy positions, after which the AI generates multiple tactical route options. Each route represents a different approach philosophy, from balanced routes that trade off speed and concealment, to stealth routes that prioritize maximum cover at the cost of longer travel times.
...
\begin{description}
    \item[Route Mode] is the primary mode for new tactical planning when you need the AI to propose approach options. You define the problem by placing friendly and enemy positions, and the AI generates multiple route options.
...
The route drawing prompt instructs Gemini 3 Pro to analyze a satellite image with marked start and end positions, then draw tactical infantry movement routes directly on the image. Key elements include:

\begin{itemize}
    \item Number of routes to generate
    \item Line colors (ORANGE for balanced, GREEN for stealth)
    \item Drawing style (smooth curves vs angular)
    \item Obstacle avoidance rules
\end{itemize}
...
Zustand provides global state management with a minimal API. The store is defined as a single hook (\texttt{useMission}) that components call to access and modify state.
